...
def display_predict_page():
    ...
    if selected_items and selected_stores:
        with col3:
            ...
            st.markdown("##### Email for Prediction Report")
            notification_email = st.text_input(
                "Email address to receive prediction report",
                value=st.session_state.get('notification_email', 'dungngo0935431740@gmail.com'),
                help="After prediction is completed, a detailed report will be sent to this email address."
            )
            # Save email to session state for reuse
        ...
        #     # Add email input for notifications
        #     st.markdown("##### Email for Prediction Report")
        #     notification_email = st.text_input(
        #         "Email address to receive prediction report",
        #         value=st.session_state.get('notification_email', 'user@example.com'),
        #         help="After prediction is completed, a detailed report will be sent to this email address."
        #     )
        #     # Save email to session state for reuse
        #     if notification_email:
                ...
                with st.spinner("Predicting sales..."):
                    ...
                    if response.status_code == 200:
                        ...
                        with st.spinner("Sending email notification..."):
                            recipient_email = st.session_state.get('notification_email', 'user@example.com')
                            success, message = send_prediction_email(results_df, recipient_email)
                            
                            if success:
                                st.success(f"Email notification sent successfully to {recipient_email}")
                            else:
                                st.warning(f"Could not send email notification: {message}")
                                st.info("To enable email notifications, you need to configure SMTP settings with a valid Gmail account and app password.")
...
def send_prediction_email(prediction_results, recipient_email="dungngo0935431740@gmail.com"):
    """
    Send email notification with prediction results.
    
    Args:
        prediction_results: DataFrame containing prediction results
    ...
    myEmail = 'resrrdttrt@gmail.com'
    # # Debug: Check if credentials are set
    # print(f"SMTP_USERNAME is set: {smtp_username if smtp_username else 'not set'}")
    # print(f"SMTP_PASSWORD is set: {smtp_password if smtp_password else 'not set'}")
    try:
        # Validate input
        if not isinstance(prediction_results, pd.DataFrame):
            print("Invalid data: prediction_results is not a pandas DataFrame")
            return False, "Error: prediction_results must be a pandas DataFrame"
        required_columns = ['Store_Id', 'Item_Id', 'Sales_Prediction']
        if not all(col in prediction_results.columns for col in required_columns):
            missing_cols = [col for col in required_columns if col not in prediction_results.columns]
            print(f"Invalid data: prediction_results missing required columns: {missing_cols}")
            return False, f"Error: prediction_results missing required columns: {missing_cols}"

        # Check if DataFrame is empty
        if prediction_results.empty:
            print("Invalid data: prediction_results is empty")
            return False, "Error: prediction_results is empty"
        ...
        # Create message
        msg = MIMEMultipart()
        msg['Subject'] = f'📊 BÁO CÁO SẢN XUẤT - KỲ {month_year}'
        msg['From'] = myEmail
        msg['To'] = recipient_email
        
        # Group results by store
        store_groups = prediction_results.groupby('Store_Id')
        store_ids = list(store_groups.groups.keys())

        # Initialize email body
        body = f"""
        <html>
        <body style="font-family: Arial, sans-serif; line-height: 1.6;">
        <h1>📋 BÁO CÁO SẢN XUẤT - KỲ {month_year}</h1>
        <hr>
        """

        # Section 1: Top performing stores
        body += "<h2>1. Thông tin cửa hàng bán chạy trong kỳ trước</h2>"
        if len(store_ids) == 0:
            body += "<p><em>Không có dữ liệu cửa hàng để hiển thị.</em></p>"
            print("Skipped top stores section: No stores available in prediction_results")
        else:
            # Compute top stores based on total sales
            top_stores = []
            for store_id, store_data in store_groups:
                total_sales = store_data['Sales_Prediction'].sum()
                top_stores.append({
                    "id": store_id,
                    "name": f"Cửa hàng {store_id}",
                    "sales": int(total_sales),
                    "region": "Unknown",  # Replace with actual region if available
                    "note": ""
                })
            # Sort by sales and take top 2 (if available)
            top_stores = sorted(top_stores, key=lambda x: x['sales'], reverse=True)[:2]
            
            if not top_stores:
                body += "<p><em>Không có dữ liệu cửa hàng để hiển thị.</em></p>"
                print("Skipped top stores section: No valid sales data for stores")
            else:
                for i, store in enumerate(top_stores, 1):
                    store['note'] = f"Bán tốt thứ {i}"
        ...
        # Section 1.5: Pie chart comparing store sales
        body += "<p>Biểu đồ tròn so sánh số lượng bán ra của các cửa hàng:</p>"
        if len(store_ids) < 1:
            body += "<p><em>Không có dữ liệu để tạo biểu đồ (cần ít nhất 1 cửa hàng).</em></p>"
            print("Skipped pie chart: Fewer than 1 store available")
        else:
            # Create a sales chart
            plt.figure(figsize=(8, 6))
            store_sales = [store_groups.get_group(sid)['Sales_Prediction'].sum() for sid in store_ids]
            store_labels = [f"Store {sid}" for sid in store_ids]
            
            if not any(sale > 0 for sale in store_sales):
                body += "<p><em>Không có dữ liệu bán hàng để tạo biểu đồ.</em></p>"
                print("Skipped pie chart: No positive sales data")
            else:
                plt.pie(store_sales, labels=store_labels, autopct='%1.1f%%')
                ...
                chart_img = MIMEImage(chart_buffer.read())
                chart_img.add_header('Content-ID', '<sales_chart>')
                msg.attach(chart_img)
                body += '<img src="cid:sales_chart" width="500">'

        # Section 2: Top 5 products per store
        body += "<h2>2. Top 5 sản phẩm bán chạy & tồn kho thấp nhất của từng cửa hàng</h2>"
        if len(store_ids) == 0:
            body += "<p><em>Không có dữ liệu cửa hàng để hiển thị.</em></p>"
            print("Skipped top products section: No stores available")
        else:
            for store_id, store_data in store_groups:
                store_name = f"Cửa hàng {store_id}"
                body += f"""
                <h3>🔹 Cửa hàng: {store_name} ({store_id})</h3>
                <table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
                    <tr style="background-color: #f2f2f2;">
                        <th>STT</th>
                        <th>Mã sản phẩm</th>
                ...
                """
                # Sort by sales prediction and take top 5
                top_products = store_data.sort_values('Sales_Prediction', ascending=False).head(5)
                
                if top_products.empty:
                    body += f"""
                    <tr>
                        <td colspan="6"><em>Không có dữ liệu sản phẩm cho cửa hàng {store_id}.</em></td>
                    </tr>
                    """
                    print(f"Skipped products for store {store_id}: No product data")
                else:
                    for i, (_, row) in enumerate(top_products.iterrows(), 1):
                        ...
                        <tr>
                            <td>{i}</td>
                            <td>{row['Item_Id']}</td>
                            <td>Sản phẩm {row['Item_Id']}</td>
                            <td>{int(row['Sales_Prediction'])}</td>
                            <td>{int(row['Sales_Prediction'] * 0.3)}</td>
                            <td>{"Cần bổ sung" if row['Sales_Prediction'] > 50 else "Đủ tồn kho"}</td>
                        </tr>
                        """
                
                body += "</table><hr>"

        # Section 3: Order predictions for next period
        body += "<h2>3. Dự đoán số lượng đặt hàng kỳ tiếp theo</h2>"
        if len(store_ids) == 0:
            body += "<p><em>Không có dữ liệu cửa hàng để hiển thị.</em></p>"
            print("Skipped order predictions section: No stores available")
        else:
            for store_id, store_data in store_groups:
                store_name = f"Cửa hàng {store_id}"
                body += f"""
                ...
                <table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
                    <tr style="background-color: #f2f2f2;">
                        <th>Mã sản phẩm</th>
                        <th>Tên sản phẩm</th>
                        <th>Số lượng đề xuất đặt sản xuất</th>
                        <th>Ghi chú</th>
                    </tr>
                """
                # Sort by predicted sales
                store_data_sorted = store_data.sort_values('Sales_Prediction', ascending=False)
                
                if store_data_sorted.empty:
                    body += f"""
                    <tr>
                        <td colspan="4"><em>Không có dữ liệu sản phẩm cho cửa hàng {store_id}.</em></td>
                    </tr>
                    """
                    print(f"Skipped order predictions for store {store_id}: No product data")
                else:
                    for _, row in store_data_sorted.iterrows():
                        priority_note = "Ưu tiên sản xuất trước" if row['Sales_Prediction'] > 50 else ""
                        body += f"""
                        <tr>
                            <td>{row['Item_Id']}</td>
                            <td>Sản phẩm {row['Item_Id']}</td>
                            <td>{int(row['Sales_Prediction'] * 1.2)}</td>
                            <td>{priority_note}</td>
                        </tr>
                        """
        ...
        body += """
        <p><em>*Note: Báo cáo này được tạo tự động sau khi hoàn tất quá trình dự đoán số lượng.</em></p>
        </body>
        </html>
        """
        ...
        attachment["Content-Disposition"] = f'attachment; filename="prediction_results_{month_year.replace("/", "_")}.csv"'
        ...
        return False, "Error: SMTP authentication failed. Check username and password."
